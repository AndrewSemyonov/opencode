...
async function createCompaction(input: Parameters<SessionCompaction.Interface["create"]>[0]) {
  const rt = runtime("continue", Plugin.defaultLayer, wide())
  try {
    await rt.runPromise(SessionCompaction.Service.use((svc) => svc.create(input)))
  } finally {
    await rt.dispose()
  }
}
